import { getAllSighConfirmationsByDayId, getAllSighConfirmationsByDayLocationId } from "@/app/database-actions";

export const calculateChanceByLocationAndDay = async (truckId: number, address: string, dayOfWeek: number): Promise<number> => {
    // Fetch all confirmations for the truck on the given day
    const allConfirmations = await getAllSighConfirmationsByDayId(truckId, dayOfWeek);
    ...
    const confirmationsByLocation = await getAllSighConfirmationsByDayLocationId(truckId, address, dayOfWeek);
